...
class Connection(ConnectionBase):
    ...
    def put_file(self, in_path, out_path):
        ...
        display.vvv(
            "PUT %s TO %s" % (in_path, out_path), host=self._remote_vmname
        )

        with open(in_path, "rb") as fobj:
            source_data = fobj.read()

        retcode, dummy, dummy = self._qubes(
    ...
    def fetch_file(self, in_path, out_path):
        ...
        display.vvv(
            "FETCH %s TO %s" % (in_path, out_path), host=self._remote_vmname
        )
        ...
        with open(out_path, "wb") as fobj:
            ...
            if p.returncode != 0:
                raise RuntimeError(
                    "Failed to fetch file to {0}".format(out_path)
                )
